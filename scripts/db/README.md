...
```bash
sqlite3 data/chroma_db/chroma.sqlite3 < scripts/db/01_inspect_schema.sql
```

### Opción 2: Ejecutar todos los scripts secuencialmente
```bash
./scripts/db/run_all_queries.sh
```

### Opción 3: Modo interactivo
```bash
sqlite3 data/chroma_db/chroma.sqlite3
sqlite> .read scripts/db/01_inspect_schema.sql
```

### Opción 4: Generar reporte completo
```bash
python scripts/db/generate_report.py
```
