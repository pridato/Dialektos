...
Salida:
    ...
    - Archivo JSON: scripts/db/duplicates_report.json
...
def main():
    ...
    OUTPUT_PATH = Path("scripts/db/duplicates_report.json")
    ...
    try:
        ...
        if stats['chunks_to_remove'] > 0:
            ...
            print("=" * 80)
            print("1. Revisar el reporte: scripts/db/duplicates_report.json")
            print("2. Hacer backup: cp -r data/chroma_db data/chroma_db.backup")
            print("3. Ejecutar limpieza: python scripts/db/cleanup_duplicates.py")
            print("=" * 80)
        else:
